...
df = spark.read.option("recursiveFileLookup", "true").csv("gs://revp1-bucket/", header=True, inferSchema=True)

# Clean the data: Remove rows with missing or empty transaction_id and status
df_clean = df.dropna().filter((trim(col("transaction_id")) != "") & (trim(col("status")) != ""))

# Save the cleaned data back to GCS 
df_clean.coalesce(1).write.csv("gs://revp1-bucket/cleaned_data_single.csv", header=True, mode="overwrite")
